...
module.exports = function() {
  ...
  sendFileICAP: function (fileName, icapDir, testsDir, inputDir, outputDir) {
    ...
    require('child_process').execSync(`cd ${icapDir} && docker run -v ${inputPath}:/opt -v ${outputPath}:/home glasswallsolutions/c-icap-client:manual-v1 -s 'gw_rebuild' -i icap-client-main.uksouth.cloudapp.azure.com -f '/opt/${fileName}' -o /home/${fileName} -v && cd ${testsDir}`, function(err, stdout, stderr) {
        if (err) {
            console.log('err ' + err);
        }
      });
